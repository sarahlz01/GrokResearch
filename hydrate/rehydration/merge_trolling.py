#!/usr/bin/env python3
"""
merge_trolling.py (MULTI)

conversation.annotations.trolling becomes an ARRAY so multiple annotations can exist:
  annotations: { trolling: [ {...}, {...} ] }

If no mapping: trolling = []
"""
...
def load_trolling_index(path: str) -> Dict[str, List[Dict[str, Any]]]:
    """
    Build:
      conversationId -> [trolling_payload, ...]
...
def main():
    ...
    ap.add_argument("--trolling", required=True)
    ap.add_argument("--output", default="dehydrated_with_trolling.json")
    ap.add_argument("--log-every", type=int, default=1000)
    args = ap.parse_args()

    trolling_by_conv = load_trolling_index(args.trolling)
    ...
    with open(args.input, "rb") as fin, open(args.output, "w", encoding="utf-8") as fout:
        ...
        for conv in ijson.items(fin, "item"):
            ...
            # always an array now
            ann["trolling"] = trolling_by_conv.get(cid, [])
